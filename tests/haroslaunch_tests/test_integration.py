...
class MockInterface(object):
    ...
    def read_text_file(self, filepath):
        ...
        if safe_dir not in filepath.parents:
            raise ValueError(filepath)
        try:
...
def test_kobuki_minimal():
    ...
    assert len(node.environment) == 0
    # Parameters ---------------------------
    da_ga = 'diagnostic_aggregator/GenericAnalyzer'
    params = {
        ...
        '/diagnostic_aggregator/analyzers/input_ports/type': da_ga,
...
def test_kobuki_safe_keyop():
    ...
    assert (
        node.args.value
        == 'load kobuki_safety_controller/SafetyControllerNodelet mobile_base_nodelet_manager'
    )
    assert node.output.value == 'log'
    assert node.working_dir.value == 'ROS_HOME'
    assert node.launch_prefix is None
    ...
    assert (
        node.remaps['/kobuki_safety_controller/cmd_vel'].get_value()
        == '/cmd_vel_mux/safety_controller'
    )
    assert (
        node.remaps['/kobuki_safety_controller/events/bumper'].get_value()
        == '/mobile_base/events/bumper'
    )
    assert (
        node.remaps['/kobuki_safety_controller/events/cliff'].get_value()
        == '/mobile_base/events/cliff'
    )
    assert (
        node.remaps['/kobuki_safety_controller/events/wheel_drop'].get_value()
    ...
    assert (
        node.args.value
        == 'load yocs_velocity_smoother/VelocitySmootherNodelet mobile_base_nodelet_manager'
    )
    assert node.output.value == 'log'
    assert node.working_dir.value == 'ROS_HOME'
    assert node.launch_prefix is None
    ...
    assert (
        node.remaps['/keyop_vel_smoother/smooth_cmd_vel'].get_value()
        == '/cmd_vel_mux/keyboard_teleop'
    )
    assert node.remaps['/keyop_vel_smoother/odometry'].get_value() == '/odom'
    assert (
        node.remaps['/keyop_vel_smoother/robot_cmd_vel'].get_value()
