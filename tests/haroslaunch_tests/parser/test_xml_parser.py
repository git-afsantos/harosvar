...
def _arg_attr_filter(attributes):
    return _any_tag_attr_filter(attributes) and not (
        'value' in attributes and 'default' in attributes
    )
...
def _param_attr_filter(attributes):
    if not _any_tag_attr_filter(attributes):
        return False
    p = [key for key in ('value', 'textfile', 'binfile', 'command') if key in attributes]
    return len(p) == 1
...
def _rosparam_attr_filter(attributes):
    ...
    if cmd == 'dump' and 'file' not in attributes:
        return False
    return cmd != 'delete' or ('param' in attributes and 'file' not in attributes)
...
def test_malformed_xml():
    try:
        ...
        raise AssertionError('<launch> is not closed')
...
def test_no_launch_root():
    try:
        ...
        raise AssertionError('no <launch> at root')
    except LaunchParserError:
        pass
...
def test_unknown_tag():
    try:
        ...
        raise AssertionError('<dummy> tag should not be accepted')
    except LaunchParserError:
        pass
...
def test_json_missing_key():
    try:
        from_JSON_object({})
        raise AssertionError('missing schema keys')
    except KeyError:
        pass
...
def test_json_bad_value_type():
    try:
        ...
        raise AssertionError('line and column should be int')
    except TypeError:
        pass
...
def test_json_unknown_tag():
    try:
        from_JSON_object(
            {'tag': 'dummy', 'text': '', 'line': 1, 'column': 1, 'attributes': {}, 'children': []}
        )
        raise AssertionError('unknown <dummy> tag')
    except KeyError:
        pass
...
def _test_valid_ast(ast):
    ...
    assert all(
        json_ast['children'][i]['tag'] == ast.children[i].tag for i in range(len(ast.children))
    )
    # --------------------------------------
