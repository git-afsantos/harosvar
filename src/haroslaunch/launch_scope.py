...
def _yaml_param(name, ns, pns, value, condition, location):
    ...
    for key, literal in _unfold(name, value):
        ...
        params.append(
            RosParameter(ros_name, v.param_type, v, condition=condition, location=location)
        )
    return params
...
class BaseScope(object):
    ...
    def _anonymous_name(self, name):
        ...
        name = '{}_{}_{}_{}'.format(
            name, socket.gethostname(), os.getpid(), random.randint(0, sys.maxsize)
        )
    ...
    def set_param(self, name, value, param_type, condition, ns='', location=None):
        assert isinstance(name, str)
        ...
        if param_type == TYPE_YAML:
            ...
            if value.is_resolved and isinstance(value.value, dict):
                ...
                params = _yaml_param(name, ns, pns, value.value, condition, location)
            else:
                params = (
                    RosParameter(
                        ros_name, param_type, value, condition=condition, location=location
                    ),
                )
        else:
            RosName.check_valid_name(name, no_ns=False, no_empty=True)
            if value.is_resolved and value.param_type != param_type:
                raise TypeError('expected {!r}, got {!r}'.format(param_type, value.param_type))
            params = (
                RosParameter(ros_name, param_type, value, condition=condition, location=location),
            )
        self._set_ros_params(params)

    def _set_ros_params(self, params):
        for param in params:
            RosName.check_valid_name(str(param.name), no_ns=False, no_empty=True)
    ...
    def new_group(self, ns, condition):
        ...
        return GroupScope(
            ...
            ns,
            dict(self.args),
            dict(self.arg_defaults),
            condition,
            self.anonymous,
            VariantDict(self.remaps),
            VariantDict(self.node_env),
            list(self.fwd_params),
            self.machines,
            self._machine,
        )
...
class LaunchScope(BaseScope):
    ...
    def __init__(
        ...
        super(LaunchScope, self).__init__(
            None,
            iface,
            ns,
            args,
...
class NodeScope(BaseScope):
    ...
    def __init__(self, node, parent, iface, args, arg_defaults, anon, machines, def_machine):
...
class IncludeScope(BaseScope):
    ...
    def __init__(
        ...
        super(IncludeScope, self).__init__(
            parent,
            iface,
            ns,
            args,
