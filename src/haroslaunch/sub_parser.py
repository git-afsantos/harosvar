...
class SubstitutionError(Exception):
    ...
    def nargs_eq(cls, cmd, n, args):
        if n == 0:
            return cls(cls._MSG_ZERO.format(cmd, args))
        else:
            return cls(cls._MSG_EQ.format(cmd, n, args))

    _MSG_GTE = '$({}) takes at least {} arguments, but received {!r}'

    @classmethod
    def nargs_gte(cls, cmd, n, args):
        return cls(cls._MSG_GTE.format(cmd, n, args))
    ...
    def nargs_lte(cls, cmd, n, args):
        if n <= 0:
            ...
            return cls(cls._MSG_LTE.format(cmd, n, args))
...
class SubstitutionParser(object):
    ...
    def resolve(self, scope):
        parts: List[Union[str, UnresolvedValue]] = []
        unknown = False
    ...
    def _build_command_list(self, value):
        self._commands: List[SubstitutionCommand] = []
        ...
        rest = value
        while match:
            parts = list(filter(bool, map(_strip, match.group(1).split(None, 1))))
            ...
            if arg_str:
                ...
                args = tuple(filter(bool, map(_strip, arg_str.split(None, 1))))
            else:
                args = ()
    ...
    def __repr__(self):
        return '{}({!r}, param_type={!r})'.format(type(self).__name__, self.text, self.param_type)
