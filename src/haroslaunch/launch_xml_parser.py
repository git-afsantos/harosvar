...
from .sub_parser import SubstitutionParser  # noqa: E402
...
class SchemaError(Exception):
    ...
    def invalid_child(cls, child_tag_name, parent_tag_name):
        return cls('<{}> cannot be a child of <{}>'.format(child_tag_name, parent_tag_name))
...
class BaseLaunchTag(object):
    CHILDREN = ()
    REQUIRED = ()
    ATTRIBUTES = {'if': TYPE_BOOL, 'unless': TYPE_BOOL}
    ENUMS = {}
...
class NodeTag(BaseLaunchTag):
    ...
    ENUMS = {'output': ('screen', 'log'), 'cwd': ('ROS_HOME', 'node')}
...
class RemapTag(BaseLaunchTag):
    REQUIRED = ('from', 'to')
    ATTRIBUTES = {'if': TYPE_BOOL, 'unless': TYPE_BOOL, 'from': TYPE_STRING, 'to': TYPE_STRING}
...
class ParamTag(BaseLaunchTag):
    ...
    ENUMS = {
        'type': (TYPE_BOOL, TYPE_STR, TYPE_STRING, TYPE_INT, TYPE_DOUBLE, TYPE_YAML, TYPE_AUTO)
    }
...
class RosparamTag(BaseLaunchTag):
    ...
    ENUMS = {'command': ('load', 'delete', 'dump')}
...
class EnvTag(BaseLaunchTag):
    REQUIRED = ('name', 'value')
    ATTRIBUTES = {'if': TYPE_BOOL, 'unless': TYPE_BOOL, 'name': TYPE_STRING, 'value': TYPE_STRING}
...
class MachineTag(BaseLaunchTag):
    ...
    def resolve_timeout(self, scope):
        result = self._resolve_attr('timeout', scope, default='10.0', no_empty=True)
...
def parse(xml_text):
    try:
        xml_root = ElementTree.fromstring(xml_text, parser=LineNumberingParser())
...
def _parse_tag(xml_tag):
    ...
    try:
        element = cls(
            text, xml_tag.attrib, xml_tag._start_line_number, xml_tag._start_column_number
        )
